-- TURN ON FOREIGN KEYS
PRAGMA foreign_keys=ON;

-- CONFIGURE DB FOR INGESTION
BEGIN IMMEDIATE;
-- COPY PUBMED TABLES
CREATE TABLE [MESH] AS SELECT * FROM PUBMED.[MESH];
CREATE TABLE [INFO] AS SELECT * FROM PUBMED.[INFO];
CREATE TABLE [IDS] AS SELECT * FROM PUBMED.[IDS];
-- COPY PMC_CAPTIONS TABLES
CREATE TABLE [CAPTIONS] AS SELECT * FROM PMC_CAPTIONS.[CAPTIONS];
-- RENAME TABLES I'M UPDATING
ALTER TABLE [CATEGORIES] RENAME TO [CATEGORIES_OLD];
ALTER TABLE [NAMES] RENAME TO [NAMES_OLD];
ALTER TABLE [SYNONYMS] RENAME TO [SYNONYMS_OLD];
ALTER TABLE [MESH] RENAME TO [MESH_OLD];
ALTER TABLE [INFO] RENAME TO [INFO_OLD];
ALTER TABLE [IDS] RENAME TO [IDS_OLD];
ALTER TABLE [CAPTIONS] RENAME TO [CAPTIONS_OLD];
-- DEFINE UPDATE TABLES AND COPY OLD VALUES INTO THEM
CREATE TABLE [CATEGORIES] (
    [ID] INTEGER PRIMARY KEY,
    [NAME] TEXT
);
INSERT INTO [CATEGORIES] ([ID], [NAME]) SELECT [ID], [NAME] FROM [CATEGORIES_OLD];
CREATE TABLE [NAMES] (
    [ID] INTEGER PRIMARY KEY,
    [CURIE] TEXT,
    FOREIGN KEY [CATEGORY] INTEGER REFERENCES [CATEGORIES]([ID]),
    [NAME] TEXT,
    [TAXON] INTEGER
);
INSERT INTO [NAMES] ([ID], [CURIE], [CATEGORY], [NAME], [TAXON]) SELECT [ID], [CURIE], [CATEGORY], [NAME], [TAXON] FROM [NAMES_OLD];
CREATE TABLE [SYNONYMS] (
    FOREIGN KEY [ID] INTEGER REFERENCES [NAMES]([ID]),
    [L1] TEXT,
    [L3] TEXT
);
INSERT INTO [SYNONYMS] ([ID], [L1], [L3]) SELECT [ID], [L1], [L3] FROM [SYNONYMS_OLD];
CREATE TABLE [MESH] (
    [PMID] INTEGER PRIMARY KEY,
    [MESH] TEXT,
    [MESH_MAJOR] TEXT
);
INSERT INTO [MESH] ([PMID], [MESH], [MESH_MAJOR]) SELECT [PMID], [MESH], [MESH_MAJOR] FROM [MESH_OLD];
CREATE TABLE [INFO] (
    FOREIGN KEY [PMID] INTEGER PRIMARY KEY REFERENCES [MESH]([PMID]),
    [FIRST_AUTHOR] TEXT,
    [YEAR] INTEGER,
    [JOURNAL] TEXT,
    [TITLE] TEXT
);
INSERT INTO [INFO] ([PMID], [FIRST_AUTHOR], [YEAR], [JOURNAL], [TITLE]) SELECT [PMID], [FIRST_AUTHOR], [YEAR], [JOURNAL], [TITLE] FROM [INFO_OLD];
CREATE TABLE [IDS] (
    FOREIGN KEY [PMID] INTEGER REFERENCES [MESH]([PMID]),
    [ALT] TEXT
);
INSERT INTO [IDS] ([PMID], [ALT]) SELECT [PMID], [ALT] FROM [IDS_OLD];
CREATE TABLE [CAPTIONS] (
    [PMC] INTEGER PRIMARY KEY,
    [FILE] TEXT,
    [CAPTION] TEXT
);
INSERT INTO [CAPTIONS] ([PMC], [FILE], [CAPTIONS]) SELECT [PMC], [FILE], [CAPTION] FROM [CAPTIONS_OLD];
-- COPY BUILD NEW INDEXES FOR TBDB
CREATE INDEX [IDX_NAMES_CATEGORY] ON [NAMES]([CATEGORY]);
CREATE INDEX [IDX_NAMES_TAXON] ON [NAMES]([TAXON]);
CREATE INDEX [IDX_SYNONYMS_ID] ON [SYNONYMS]([ID]);
CREATE INDEX [IDX_SYNONYMS_L1] ON [SYNONYMS]([L1]);
CREATE INDEX [IDX_SYNONYMS_L3] ON [SYNONYMS]([L3]);
CREATE INDEX [IDX_MESH_MESH_MAJOR] ON [MESH]([MESH_MAJOR]);
CREATE INDEX [IDX_IDS_PMID] ON [IDS]([PMID]);
CREATE INDEX [IDX_IDS_ALT] ON [IDS]([ALT]);
CREATE INDEX [IDX_CAPTIONS_FILE] ON [CAPTIONS]([FILE]);
-- COMMIT CHANGES
COMMIT;

-- OPTIMIZE DB AND FREE UP SPACE
ANALYZE;
PRAGMA optimize;
VACUUM;

-- REMOVE EXCLUSIVE LOCKING
PRAGMA locking_mode=NORMAL;